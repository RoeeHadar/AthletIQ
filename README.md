...
Not MVP-complete: no claim of remote CI green, two live NBA seasons, or ML-005 on a real holdout. Fixture provider needs **no** `API_SPORTS_KEY`.
...
- `GET http://127.0.0.1:8000/v1/model`
...
Clean-machine success of this path is **NFR-001** (owner attestation or later smoke). It is **not** attested by TEST-013 (synthetic train-repeat) or TEST-010 (static Compose topology).
...
**Gates 0–5 + Gate 7 Approved.** Gate 6: **IMP-001…012 Done**. Post-IMP remainings closed locally (Postgres adapters behind `--store` / `ATHLETIQ_STORE`, TEST-013/014 Passing). Remote CI green deferred.
